...
function getTicketDataById({ credentials, baseUrl, ticketId }) {
  ...
  const ticketCommentsUri = `${baseUrl}/tickets/${ticketId}/comments.json?sort_order=desc&per_page=1`;
  ...
  return Promise.all([
    ...
    request({ json, headers, uri: ticketCommentsUri })
      .then(response => response.comments[0])
      .then((comments) => {
        debug('botmaster-zendesk: got comments ', comments);
        return comments.attachments.map(formatAttachments);
      }),
  ]);
}
